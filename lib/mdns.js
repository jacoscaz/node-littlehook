var mdns          = require('mdns'),
    ...
    debug         = require('debug')('littlehook:mdns');
...
module.exports.advertRestart = function(hook) {
    debug(hook.name, ' scheduled an advert restart');
    hook.mdns.timeout = hook.mdns.timeout || setTimeout(function() {
        ...
        debug('MDNS advert for ', hook.name, ' has been restarted');
    }, 1000);
}

module.exports.advertHasBeenStarted = function(hook) {
    return (hook.mdns.advert && hook.mdns.advert._watcherStarted);
}

module.exports.browserStart = function(hook) {
    var browser = hook.mdns.browser = mdns.createBrowser(mdns.tcp('littlehook'));
...
module.exports.browserOnUp = function(hook, service) {
    ...
    debug(hook.name, ' has detected a change in ', service.name, ' [' + event + ']');
    hook.peers[service.name] = 
        (hook.peers[service.name] || new Peer(hook)).update(service);
...
module.exports.browserOnDown = function(hook, service) {
    ...
    hook.peers[service.name].timeout = setTimeout(function() {
        ...
        process.nextTick(function() {
            debug(hook.name, ' has detected a change in ', service.name, ' [down]');
            EventEmitter2.prototype.emit.call(hook, [
                service.name,
                'down'
            ]);
