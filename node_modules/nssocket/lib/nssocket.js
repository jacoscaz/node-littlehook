...
var NsSocket = exports.NsSocket = function (socket, options) {
  ...
  // J: "connected" & "disconnected" events hack
  // this.connected  = options.connected || socket.writable && socket.readable || false;
  var __connected = options.connected || socket.writable && socket.readable || false;
  this.__defineGetter__('connected', function(){return __connected;});
  this.__defineSetter__('connected', function(value){
    __connected = value;
    (value) ? this.emit('connected') : this.emit('disconnected');
  });
